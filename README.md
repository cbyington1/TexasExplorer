...
Mobile map can drift when pinch-zooming at extreme zoom levels (Leaflet maxBounds limitation)
Diversity index is NULL for many small rural cities (by design - insufficient data)
JPA flush timing requires native SQL for batch deletes (deleteAllByYear)
Trend sliders use symmetric ranges (±100 or ±500) centered on zero for UX clarity
